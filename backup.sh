...
#time pg_dump -b -F c --dbname=$DATABASE_URL >  /tmp/"${DBNAME}_v_${FILENAME}".gz
...
#time /app/vendor/awscli/bin/aws s3 cp /tmp/"${DBNAME}_v_${FILENAME}" s3://$S3_BUCKET_PATH/$DBNAME/"${DBNAME}_${FILENAME}" --expires $EXPIRATION_DATE
# cleaning after all
